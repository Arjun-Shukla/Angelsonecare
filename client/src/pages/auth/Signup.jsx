...
function ErrorBanner({ message }) {
  ...
  return (
    <div role="alert" className="mb-5 flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3.5">
      <svg className="mt-0.5 w-5 h-5 shrink-0 text-red-500" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
...
function SuccessScreen({ name, email, showToast }) {
  return (
    <div className="text-center py-4">
      <EmailSentToast
        show={showToast}
        message="Welcome email sent!"
        detail={`Check your inbox at ${email}`}
      />
      ...
      <h2 className="text-xl font-bold text-slate-800 mb-1">Welcome, {name.split(' ')[0]}!</h2>
      <p className="text-slate-500 text-sm mb-4 leading-relaxed">
        Your account has been created. A welcome email has been sent to{' '}
...
export default function Signup() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        setAuthError(json.message ?? 'Registration failed. Please try again.');
        setLoading(false);
        return;
      }
      ...
      setUser(user);
      setSuccessData({ name: user.name, email: user.email });
      setShowSuccess(true);
  ...
  return (
    <AuthLayout
      heading={showSuccess ? "You're all set!" : "Create your account"}
      subheading={
        showSuccess
