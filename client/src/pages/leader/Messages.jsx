  ...
  return (
    <div
      className={`bg-white rounded-2xl border p-5 transition-all ${
        msg.isRead ? 'border-slate-100' : 'border-emerald-200 shadow-sm'
      }`}
    >
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div className="flex items-start gap-3 flex-1 min-w-0">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
            msg.isRead ? 'bg-slate-100' : 'bg-emerald-100'
          }`}>
            <EnvelopeIcon className={`w-5 h-5 ${msg.isRead ? 'text-slate-400' : 'text-emerald-600'}`} />
          </div>
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              <p className="text-sm font-bold text-slate-900">{msg.name}</p>
              {!msg.isRead && (
                <span className="text-[10px] font-bold bg-emerald-600 text-white px-2 py-0.5 rounded-full">NEW</span>
              )}
              {msg.service && (
      ...
      <div>
        ...
        {!loading && (
          <div className="flex gap-2 mt-2">
            <span className="text-xs font-semibold bg-emerald-100 text-emerald-700 px-2.5 py-1 rounded-full">Total: {messages.length}</span>
            {unreadCount > 0 && (
